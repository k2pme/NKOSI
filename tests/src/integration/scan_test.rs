...
pub fn test_scan_webshell_pattern() {
    ...
    assert!(
        !detections.is_empty(),
        "YARA should detect webshell pattern"
    );
    assert_eq!(
        detections[0].detection_engine,
        nkosi_common::types::DetectionEngine::Yara
    );
}
...
pub fn test_scan_clean_file() {
    ...
    assert!(
        detections.is_empty(),
        "Clean file should have no YARA detections"
    );
    ...
    assert!(
        detection.is_none(),
        "Clean file should have no hash detection"
    );
}
...
pub fn test_static_analysis_suspicious() {
    ...
    if let Some(detection) = result {
        assert_eq!(
            ...
            nkosi_common::types::DetectionEngine::StaticAnalysis
        );
        assert!(
            detection.score_contribution > 0,
            "Should have positive risk score"
        );
    }
}
...
pub fn test_static_analysis_clean() {
    ...
    if let Some(detection) = result {
        assert!(
            detection.score_contribution < 30,
            "Clean file should have low risk"
        );
    }
}
