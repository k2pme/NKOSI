...
fn ui(f: &mut Frame, app: &mut App) {
    ...
    let sidebar =
        List::new(sidebar_items).block(Block::default().borders(Borders::ALL).title("Menu"));
    ...
    let footer_text =
        "q: Quit | Tab: Switch | j/k: Navigate | ←/→: Scan path | s: Scan | r: Refresh | l: Logs";
...
fn render_dashboard(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let stats_text = format!(
        "Statistiques:\n  Événements: {}\n  Menaces: {}\n  Quarantaine: {}",
        app.stats.total_events, app.stats.total_threats, app.stats.quarantine_items
    );
...
fn render_quarantine(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let list = List::new(items).block(Block::default().borders(Borders::ALL).title("Quarantine"));
    ...
    let items: Vec<ListItem> = app
        ...
        .map(|log| {
            ListItem::new(Line::from(vec![
                ...
                Span::styled(log.source_module.clone(), Style::default().fg(Color::White)),
            ]))
        })
        .collect();
...
fn render_agents(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let online = app
        .agents
        .iter()
        .filter(|a| a.status == nkosi_common::types::AgentStatus::Online)
        .count();
