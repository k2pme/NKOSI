...
impl KernelScanner {
    ...
    pub fn scan(&self) -> Result<KernelReport> {
        ...
        info!(
            "Kernel scan completed: {} findings, score: {}",
            findings.len(),
            score
        );
    ...
    fn generate_summary(
        ...
        if findings.is_empty() {
            format!(
                "Kernel clean. {} modules loaded. No suspicious modules detected.",
                self.whitelisted_modules.len()
            )
        } else {
